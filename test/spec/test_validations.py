...
class TestNestedReferences(unittest.TestCase):
    """Test how firestone treats a reference inside an embedded resource."""

    @staticmethod
    def _nested() -> dict:
        """Get a resource whose embedded 'person' schema carries a reference."""
        rsrc = _resource()
        rsrc["methods"]["instance_attrs"] = ["get", "put"]
        rsrc["schema"]["items"]["properties"]["person"] = {
            "schema": {
                "type": "array",
    ...
    def test_it_is_not_enforced_on_the_embedding_resource(self):
        """A reference belongs to the resource that declares it, and says so."""
        with self.assertLogs("firestone.spec.validations", level="WARNING") as logs:
            ruleset = validations.extract([self._nested()])

        self.assertEqual(ruleset, {})
        self.assertIn("person.badge", logs.output[0])
        self.assertIn("is not enforced when validating addressbook", logs.output[0])

    def test_it_is_still_stripped(self):
        """It is a firestone concept wherever it sits, so it never reaches a generator."""
        cleaned = validations.strip([self._nested()])
        badge = cleaned[0]["schema"]["items"]["properties"]["person"]["schema"]["items"][
            "properties"
        ]["badge"]

        self.assertNotIn("references", badge)

    def test_it_does_not_reach_the_openapi_components(self):
        """The embedded schema is copied into components, so it has to be clean."""
        spec = openapi.generate([self._nested()], "title", "desc", "summary", "1.0")
...
class TestGenerate(unittest.TestCase):
    ...
    def test_rules_survive_being_written_out(self):
        """A rule containing quotes, backslashes or newlines comes back unchanged.

        CEL string literals use double quotes, so an expression such as
        ``matches("^\\d+$")`` is ordinary content, and a literal that re-parses to
        something else would silently change what the rule means.
        """
        expr = 'self.code.matches("^\\d+$")'
        message = 'Bad code "x"\nsecond line, 100% \\ backslash'
        ruleset = validations.extract(
            [_with_rule({"name": "tricky", "expr": expr, "error": {"message": message}})]
        )

        namespace = {}
        exec(validations.generate(ruleset)["rules.py"], namespace)  # pylint: disable=exec-used
        rule = namespace["RULES"]["addressbook"][0]

        self.assertEqual(rule["expr"], expr)
        self.assertEqual(rule["error"]["message"], message)
...
class TestGeneratedRuntime(unittest.TestCase):
    ...
    def _rules(self, rule: dict) -> dict:
        """Run one hand built rule through the generated engine."""
        return {"thing": [{"resource": "thing", "field": None, "examples": [], **rule}]}

    def test_a_skipped_rule_queues_no_lookups(self):
        """A rule skipped on its second lookup does not leave the first one queued.

        Nothing is going to run, so there is nothing to fetch, and asking for a
        resolver here would be a 500 where the answer is 'nothing to validate'.
        """
        rules = self._rules(
            {
                "name": "two_lookups",
        ...
        asyncio.run(
            self.genval.validate_with_rules(
                rules, "post", "thing", body={"present": "x"}, resolver=None
            )
        )

    def test_message_renders_values_as_json(self):
        """A placeholder reads the same whichever language rendered it.

        The rust package asserts this exact string too, so a boolean has to come out
        ...
        body = {"flag": False, "nothing": None, "items": ["a", "b"], "text": "plain"}

        with self.assertRaises(self.genval.ValidationError) as ctx:
            asyncio.run(self.genval.validate_with_rules(rules, "post", "thing", body=body))

        self.assertEqual(
            ctx.exception.violations[0].message, 'false and null and ["a","b"] and plain'
        )

    def test_a_non_boolean_expression_is_a_broken_rule(self):
        """A rule has to decide, so a truthy string is a bug rather than a pass."""
        rules = self._rules(
            {
                "name": "not_a_bool",
                "kind": "expr",
        ...
        with self.assertRaises(self.genval.RuleEvaluationError) as ctx:
            asyncio.run(self.genval.validate_with_rules(rules, "post", "thing", body={}))

        self.assertIn("rather than a boolean", str(ctx.exception))
