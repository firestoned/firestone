...
CEL is what makes that possible: `cel-python` and `cel-interpreter` both implement the same specification, so an expression means the same thing on both sides. The engines are held to that as well: an expression must evaluate to a boolean in both, and a placeholder renders a value as JSON in both, so `{self.is_valid}` reads as `true` whichever package wrote the message.
