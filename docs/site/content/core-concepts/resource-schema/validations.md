...
| `expr` | yes | | A CEL expression that must evaluate to a boolean, and must be true for the request to be accepted |
| `error.status` | no | `422` | The HTTP status to respond with |
| `error.message` | no | generated | The message, with optional `{self.city}` style placeholders. Strings interpolate unquoted; anything else is rendered as JSON, so a boolean reads as `true` |
...
An expression has to evaluate to a **boolean**. A rule that returns a string or a number is a bug rather than a quietly truthy pass, and both runtimes refuse it as a `RuleEvaluationError`.
...
## A reference is enforced on the resource that declares it

`references` is read from the top level of a resource's own `items.properties`. A `$ref` to another resource is resolved before firestone sees it, so a reference declared inside `person.yaml` appears again, nested, under every resource that embeds a person - but it is enforced when validating `persons`, not when validating `addressbook`. Firestone says so rather than dropping it quietly:

```
WARNING addressbook: the 'references' on person.badge is inside an embedded
        resource, so it is not enforced when validating addressbook. A reference
        is only enforced on the resource that declares it.
```

To constrain an embedded value from the embedding resource, write a rule there with an explicit `value` path:

```yaml
validations:
  rules:
    - name: badge_must_exist
      refs:
        badge: { kind: badges, key: id, value: person.badge }
      expr: has(refs.badge)
```
