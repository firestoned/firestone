...
def _iter_properties(properties: dict, prefix: str = ""):
    """Walk a resource's properties, descending into embedded resource schemas.

    A ``$ref`` to another resource is resolved before firestone sees it, so an
    embedded resource arrives as a nested ``schema.items.properties`` of its own.
    Anything firestone owns has to be found there too, not just at the top level.

    :param dict properties: the properties to walk
    :param str prefix: the dotted path the properties sit under
    :return: yields the dotted path and the schema of every property found
    """
    for name in properties:
        ...
        if isinstance(items, dict) and isinstance(items.get("properties"), dict):
            yield from _iter_properties(items["properties"], f"{path}.")
...
def extract(rsrc_data: list) -> dict:
    ...
    for rsrc in rsrc_data:
        ...
        # A reference is enforced on the resource that declares it. One sitting inside
        # an embedded resource belongs to that resource, so say so rather than drop it
        # without a word.
        for path, prop_schema in _iter_properties(properties):
            if "." in path and "references" in prop_schema:
                _LOGGER.warning(
                    f"{rsrc_kind}: the 'references' on {path} is inside an embedded "
                    f"resource, so it is not enforced when validating {rsrc_kind}. A "
                    "reference is only enforced on the resource that declares it."
                )
...
def strip(rsrc_data: list) -> list:
    ...
    for rsrc in cleaned:
        ...
        properties = rsrc.get("schema", {}).get("items", {}).get("properties", {})
        # Embedded resources included: a reference left behind in one of those would
        # be copied straight into the components of the OpenAPI document.
        for _, prop_schema in _iter_properties(properties):
            prop_schema.pop("references", None)
...
def _string(value: str) -> str:
    """Quote a string as a python literal, the way black would.

    Both branches escape properly: a rule is free to contain a backslash, a quote
    or a newline, and CEL string literals use double quotes, so an expression such
    as ``matches("^\\d+$")`` has to survive being written out and read back
    unchanged.
    """
    if '"' in value and "'" not in value:
        # repr picks single quotes for exactly this case, which is black's
        # preference too, and escapes everything else correctly.
        return repr(value)
